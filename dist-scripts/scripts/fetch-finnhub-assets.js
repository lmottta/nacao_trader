...
function mapFinnhubType(finnhubType) {
    ...
    if (typeof finnhubType === 'string') {
        ...
        if (lowerType.includes('stock') || lowerType.includes('etp'))
            return 'stock';
        if (lowerType.includes('forex'))
            return 'forex';
...
async function fetchForexSymbols() {
...
async function fetchCryptoSymbols(exchange = 'binance') {
...
async function syncAssets(supabase) {
    ...
    const assetsToUpsert = allSymbols
        .map(symbol => ({
...
//# sourceMappingURL=fetch-finnhub-assets.js.map